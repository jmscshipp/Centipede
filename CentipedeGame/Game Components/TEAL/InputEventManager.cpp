...
void InputEventManager::ProcessInputEvents()
{
	...
	while ( WindowManager::Window().pollEvent(winEvent))
    {
		switch ( winEvent.type )
		{
			//case sf::Event::Closed: WindowManager::Window().close(); 	break;
			...
			case sf::Event::Closed:
				if (DistpatchMap[InputFlags::Closed].size() == 0)
					WindowManager::Window().close(); // Automatic window close if no gameobject registered for this event
				else
