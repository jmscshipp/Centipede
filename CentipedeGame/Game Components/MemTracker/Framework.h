...
#pragma warning( pop )
	...
	void privDisplayLogLink()
	{
		...
		this->privOut("          %s(1) : <Click this line>\n", pBuff);
	}
			...
			while (pTmp != nullptr)
			{
				if (pTmp->nBlockUse == _NORMAL_BLOCK)
				{
					...
					else
					{
						...
						// Andre: uncomment the next line... I think it looks bad
						fprintf(pFile,"Leak(%d)  %d bytes ref:%d %s \n", (int)i, (int)pTmp->nDataSize, (int)pTmp->lRequest, pTrace->privStripDir(pTmp->szFileName));
						pTrace->privOut("Leak(%d)  %d bytes ref:%d %s \n", i, pTmp->nDataSize, pTmp->lRequest, pTrace->privStripDir(pTmp->szFileName));
						pTrace->privOut("   %s(%d) : <Click this line> \n", pTmp->szFileName, pTmp->nLine);
						pTrace->privOut("\n");
					}
...
private:
	...
	void privOpen(const char * const pFirstName, const char * const pLastName) noexcept
	{
		...
		// "_" is 1 and "/0" is 1 so add 2 bytes
		assert((strlen(pFirstName) + strlen(pLastName) + 2) < length);
