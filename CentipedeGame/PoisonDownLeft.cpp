...
const CentipedeState* PoisonDownLeft::GetNextState(CentipedeHead* head) const
{
...
sf::Vector2f PoisonDownLeft::GetPositionOffset(int index) const
{
	return positionOffset[index];
}

float PoisonDownLeft::GetRotationOffset(int index) const
{
	return rotationOffset[index];
}

void PoisonDownLeft::DebugName() const
{
	Trace::out("PoisonDownLeft\n");
}