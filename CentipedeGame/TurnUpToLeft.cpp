...
const CentipedeState* TurnUpToLeft::GetNextState(CentipedeHead* head) const
{
...
sf::Vector2f TurnUpToLeft::GetPositionOffset(int index) const
{
	return positionOffset[index];
}

float TurnUpToLeft::GetRotationOffset(int index) const
{
	return rotationOffset[index];
}

void TurnUpToLeft::DebugName() const
{
	Trace::out("TurnUpLeft\n");
}